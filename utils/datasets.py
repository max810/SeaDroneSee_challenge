...
class LoadImagesAndLabels(Dataset):
    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            ...
            r_vert, r_horiz = img_size[0] / h0, img_size[1] / w0  # ratio
            if not(r_vert == r_horiz == 1):  # if sizes are not equal
                r_avg = (r_vert + r_horiz) / 2
                im = cv2.resize(im,
                                (int(w0 * r_horiz), int(h0 * r_vert)),
                                interpolation=cv2.INTER_LINEAR if (self.augment or r_avg > 1) else cv2.INTER_AREA)
    ...
    def load_mosaic(self, index):
        ...
        if type(self.img_size) is int:
            ...
            s_h, s_w = self.img_size[0:2] 
        yc, xc = (int(random.uniform(-x, 2 * s + x)) for s, x in zip([s_h, s_w], self.mosaic_border))  # mosaic center x, y
        ...
        for x in (labels4[:, 1:], *segments4):
            np.clip(x, 0, 2 * min(s_h, s_w), out=x)  # clip when using random_perspective()
